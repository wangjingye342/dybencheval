...
# ================= Prompt 模板 (已修改：全部示例均为 Target Match) =================
...
def generate_prompt_content(loader, target_scenario, target_task):
    ...
    # 修改核心逻辑：只在同任务、同情境下采样 5 次
    shots = []
    for _ in range(5):
        shots.append(loader.get_random_sample_raw(target_scenario, target_task))
    ...
    return PROMPT_TEMPLATE.format(
        ...
        # 填充5个同源示例
        shot1_content=shots[0],
        shot2_content=shots[1],
        shot3_content=shots[2],
        shot4_content=shots[3],
        shot5_content=shots[4]
    )


def main():
    loader = DatasetLoader(DATA_ROOT)
    output_file = f"prompts_onlyself/generated_prompts_{target_scenarios[0]}_{target_tasks[0]}.jsonl"
