...
### Example 3 (Same Scenario, Different Task)
[Scenario: {target_scenario_disp}] | [Task: {shot3_task_disp}]
{shot3_content}
...
def generate_prompt_content(loader, target_scenario, target_task):
    ...
    # 2. Same Task, Different Scenario (只采1个)
    logic_s = weighted_sample(SCENARIO_PROBS[target_scenario])
    s2 = loader.get_random_sample_raw(logic_s, target_task)

    # 3. Same Scenario, Different Task (只采1个)
    style_t = weighted_sample(TASK_PROBS[target_task])
    s3 = loader.get_random_sample_raw(target_scenario, style_t)
    ...
    return PROMPT_TEMPLATE.format(
        ...
        # Shot 2 (Different Scenario)
        shot2_scenario_disp=SCENARIO_DISPLAY[logic_s],
        shot2_content=s2,

        # Shot 3 (Different Task)
        shot3_task_disp=TASK_DISPLAY[style_t],
        shot3_content=s3
    )
...
def main():
    ...
    output_file = f"prompts_3/generated_prompts_{target_scenarios[0]}_{target_tasks[0]}.jsonl"
