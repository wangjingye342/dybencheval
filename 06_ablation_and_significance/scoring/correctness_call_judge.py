...
def process_single_line(line):
    ...
    try:
        ...
        # 构造输出数据
        # 注意：这里根据 'Correctness Prompt' 生成时的字段进行了调整
        output_data = {
            "id": data.get("id"),
...
def process_datasets():
    ...
    for filename in files:
        ...
        # 构造输出文件名： xxx_prompt_correctness.jsonl -> xxx_result_correctness.jsonl
        # 这样命名更清晰
        file_name_only = filename.replace("_prompt_correctness.jsonl", "")
        if file_name_only == filename:  # 如果没替换成功（后缀不匹配），就直接去后缀
            file_name_only = os.path.splitext(filename)[0]

        new_filename = f"{file_name_only}_result_correctness.jsonl"
        ...
        with open(output_path, 'a', encoding='utf-8') as outfile:
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                ...
                for future in tqdm(concurrent.futures.as_completed(future_to_line), total=total_tasks,
                                   desc="Evaluating correctness", unit="lines"):
