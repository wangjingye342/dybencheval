...
def process_dataset(input_path, output_path, map_part1, map_part2, limit):
    output_dir = os.path.dirname(output_path)
    if output_dir and not os.path.exists(output_dir):
        os.makedirs(output_dir)

    print(f"开始处理大文件，目标输出路径: {output_path}")
    print(f"前 {limit} 行使用索引1，剩余行使用索引2")
...
if __name__ == "__main__":
    ...
    # 3. 处理文件，传入两个索引
    process_dataset(INPUT_BIG_FILE, OUTPUT_FILE, index1, index2, LIMIT_ROWS)