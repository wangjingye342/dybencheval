...
INPUT_DIRECTORY = Path("./external/metrics/1/re_embedding")
# 新的输出目录，用于保存带有similarity得分的数据
OUTPUT_DIRECTORY = Path("./external/metrics/1/scored_data")
...
def process_dataset(input_path: Path, output_path: Path):
    ...
    if valid_count > 0:
        ...
        print(f" -> [{input_path.name}] 平均similarity: {avg_score:.4f} (共 {valid_count} 条)")
